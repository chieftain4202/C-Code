...
void TIM3_Out_Freq_Generation(unsigned short freq)
{
	...
	unsigned int arr;
	unsigned int ccr;
	psc = (TIMXCLK / TIM3_FREQ + 0.5) - 1;
	arr = (TIM3_FREQ / freq + 0.5) - 1;
	ccr = (arr + 1) / 2;
	// Timer 주파수가 TIM3_FREQ가 되도록 PSC 설정
	Macro_Write_Block(TIM3->PSC, 0xffff, psc, 0);
	// 요청한 주파수가 되도록 ARR 설정
	Macro_Write_Block(TIM3->ARR, 0xffff, arr, 0);
	// Duty Rate 50%가 되도록 CCR3 설정
	Macro_Write_Block(TIM3->CCR3, 0xffff, ccr, 0);
	// Manual Update(UG 발생)
